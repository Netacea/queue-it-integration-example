...
import { CloudflareCryptoProvider } from "./cryptoProvider";

export default class CloudflareHttpContextProvider implements IConnectorContextProvider {
  _httpRequest: RequestProvider;
  _httpResponse: ResponseProvider;
  _outputCookie: string;
  _enqueueTokenProvider: IEnqueueTokenProvider;
  ...
  constructor(public request: any, public bodyString: string) {
    ...
    this._cryptoProvider = new CloudflareCryptoProvider();
  }
  ...
  public setEnqueueTokenProvider(
    ...
    this._enqueueTokenProvider = new DefaultEnqueueTokenProvider(customerId, secretKey, validityTime, clientIp, withKey, Token, Payload);
  }
